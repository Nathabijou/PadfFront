...
export default function PetitProjet() {
  ...
  const loadPetitProjet = async () => {
    const result = await axios.get('http://localhost:8080/petitprojet');
    ...
    <div className="container">
      <h2 className="text-center">Liste des Projets</h2>
        <div className="py-4">
          ...
          <Link  to={"/program/1/projectcomponent/1"} > 
          ...
          <tbody className="table-group-divider">
                   {petitprojet.map((petitprojet, index) => (
                  <tr key={index}>
                    <th scope="row">{index + 1}</th>
                    <td>                     
                    <Link to={`/projectcomponent/petitprojet/beneficiaire/${petitprojet.id}`}>{petitprojet.nom} </Link>                    
                      </td>
                    <td>{petitprojet.description}</td>
                    <td>{petitprojet.statut}</td>
                    <td>{petitprojet.zone}</td>
                    <td>{petitprojet.quartier}</td>
                  <td>
                         
            <Link
                  to={`/petitprojet/viewpetitprojet/${petitprojet.id}`} > 
            ...
            <Link  
                  to={`/petitprojet/editpetitprojet/${petitprojet.id}`}>
