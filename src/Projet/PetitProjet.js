...
export default function PetitProjet() {
  ...
  const loadPetitsProjets = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/typeprojet/${typeprojetId}/petitprojet`);
      ...
      <div className="py-9 mt-4 mx-4 dx-3 beneficiaire-list">
        <div className="sticky-buttons d-flex justify-content-between">
          ...
          <Link
            ...
            to={`/App/program/${programId}/composante/${composanteId}/typeprojet/${typeprojetId}/petitprojet/${petitprojetId}/addpetitProjet`}
          ...
          <tbody className="table-group-divider">
            {filteredPetitsProjets.map((petitProjet, index) => (
              <tr key={petitProjet.id}>
                ...
                <td>
                  <Link
                    to={`/App/typeprojet/${typeprojetId}/petitprojet/${petitProjet.id}/beneficiaire`}
                  >
                    {petitProjet.nom}
                  </Link>
                </td>
                ...
                <td>
                  <Link to={`/petitprojet/viewpetitprojet/${petitProjet.id}`}>
                    <svg
                  ...
                  <Link to={`/petitprojet/editpetitprojet/${petitProjet.id}`}>
                  <svg
                      width="40"
                      height="16"
                      fill="currentColor"
